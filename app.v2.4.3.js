...
function bindGlobal(){
  ...
  document.addEventListener('pointerdown',event=>{if(!event.target.closest('#exportMenu'))closeExportMenu()});
  document.addEventListener('keydown',event=>{if(event.key==='Escape'){closeExportMenu();if(!byId('modal').classList.contains('hidden'))closeModal()}});
}
