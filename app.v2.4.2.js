...
function insertTable(){
  ...
  const block={id:uid('tbl'),type:'table',code:code.trim(),title:title.trim(),caption:caption.trim(),widths:Array(cols).fill(100/cols),rows:Array.from({length:rows},()=>Array.from({length:cols},()=>({runs:[]})))},arr=state.current.content.blocks;let idx=arr.findIndex(x=>x.id===state.activeBlockId);if(idx<0)idx=arr.length-1;arr.splice(idx+1,0,block);const next=arr[idx+2];if(!next||!['paragraph','bullet','number'].includes(next.type))arr.splice(idx+2,0,normalBlock());markDirty(false);pushHistory('Insert table');renderBlocks();renderOutline();renderRegisters();setActiveBlock(block.id)
}
