...
const state = {
  ...
  ribbonTab:'home', viewOnly:false, viewZoom:1, documentActionMenu:null
};
...
async function openDocument(id){const d=await idbGet('documents',id);if(!d)return;state.current=d;state.dirty=false;state.editRevision=0;state.savedRevision=0;state.ribbonTab='home';state.viewOnly=false;state.viewZoom=1;state.activeTableId=null;state.activeCell=null;resetHistory();await activateView('editor');renderEditor();pushHistory('Open');}
...
function bindTextControls(){
  ...
  byId('viewOnlyBtn').onclick=()=>{state.viewOnly=!state.viewOnly;if(state.viewOnly){state.ribbonTab='view';state.viewZoom=.9;setOutlineDrawerOpen(false)}applyViewOnlyState();setRibbonTab(state.viewOnly?'view':'home');requestAnimationFrame(updatePageScale)};
  ...
  byId('onePageBtn').onclick=()=>{byId('onePageBtn').setAttribute('aria-pressed','true');byId('twoPagesBtn').setAttribute('aria-pressed','false');setViewZoom(1)};
  byId('twoPagesBtn').onclick=()=>{byId('onePageBtn').setAttribute('aria-pressed','false');byId('twoPagesBtn').setAttribute('aria-pressed','true');setViewZoom(.55);toast('Two-page fit uses display zoom only; document dimensions stay unchanged')};
}
...
function applyViewOnlyState(){const area=byId('textEditorArea'),button=byId('viewOnlyBtn');if(!area||!button)return;area.classList.toggle('view-only-mode',state.viewOnly);button.setAttribute('aria-pressed',String(state.viewOnly));button.classList.toggle('primary',state.viewOnly);button.textContent=state.viewOnly?'✓ View only':'◉ View only';area.querySelectorAll('.editable,.table-cell-editor').forEach(el=>el.contentEditable=state.viewOnly?'false':'true');area.querySelectorAll('.draft-information-card input,.draft-information-card textarea,.table-information-card input').forEach(el=>el.readOnly=state.viewOnly);$$('[data-ribbon-tab="home"],[data-ribbon-tab="insert"],[data-ribbon-tab="table"]').forEach(tab=>tab.disabled=state.viewOnly||(tab.dataset.ribbonTab==='table'&&!(state.activeTableId&&state.activeCell)))}
function setViewZoom(value){state.viewZoom=Math.max(.5,Math.min(1.5,Math.round(value*10)/10));const label=byId('zoomResetBtn');if(label)label.textContent=`${Math.round(state.viewZoom*100)}%`;requestAnimationFrame(updatePageScale)}
...
function scrollToBlock(id){const el=document.querySelector(`[data-block-id="${CSS.escape(id)}"]`);if(el)el.scrollIntoView({behavior:'smooth',block:'start'});if(innerWidth<=820)setOutlineDrawerOpen(false)}
