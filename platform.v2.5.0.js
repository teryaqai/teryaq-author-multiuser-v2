/* TERYAQ Master Tool Platform v2.5.0
...
async function completeInvitePassword(invite,password){
  ...
  let profile=null;try{const rows=await apiFetch(`/rest/v1/profiles?id=eq.${encodeURIComponent(authUser.id)}&select=id,email,display_name,role`);profile=Array.isArray(rows)?rows[0]:null}catch{}
  currentUser={id:authUser.id,email:authUser.email||'',displayName:profile?.display_name||authUser.user_metadata?.name||'',role:profile?.role||'user',lastAuthenticatedAt:now()};await put('accounts',currentUser);await activateOwnAccountRequest();await registerDevice();await claimAndMigrateLegacyData();await reconcileTrashedConflicts();history.replaceState(null,document.title,location.pathname+location.search);return currentUser
}
async function submitAccountRequest(request){
  const payload={p_name:String(request.name||'').trim(),p_email:String(request.email||'').trim(),p_executor_type:String(request.executor_type||'').trim(),p_note:String(request.note||'').trim()};
...
async function decideAccountRequest(requestId,action,decisionNote=''){
  ...
  return apiFetch('/functions/v1/admin-account-request',{method:'POST',body:{request_id:requestId,action,decision_note:decisionNote}})
}
...
async function showAdminDashboard(tab='overview',target='adminPageBody'){
  ...
  const body=byId(target);if(!body)return;
  if(!isOnline()){body.innerHTML='<div class="issue warn">The admin workspace needs an internet connection because it displays synchronized cloud data.</div>';return}
  body.innerHTML='<div class="page-form-card">Loading admin data…</div>';
  try{
    const [profiles,docs]=await Promise.all([
      apiFetch('/rest/v1/profiles?select=id,email,display_name,role&order=email.asc'),
      apiFetch('/rest/v1/documents?select=id,owner_id,title,template_id,template_version,current_version,updated_at,deleted_at,document_json&order=updated_at.desc')
    ]),pmap=new Map((profiles||[]).map(p=>[p.id,p])),active=(docs||[]).filter(d=>!d.deleted_at),deleted=(docs||[]).filter(d=>d.deleted_at),tabs=[
      ['overview','Overview'],['analytics','Analytics'],['content','Content Options'],['users','Users'],['documents','Documents'],['versions','Versions'],['conflicts','Cloud Conflicts'],['requests','Account Requests'],['audit','Audit Log'],['trash','All Trash']
    ];
    body.innerHTML=`<div class="admin-tabs">${tabs.map(([id,label])=>`<button class="admin-tab ${tab===id?'active':''}" data-admin-tab="${id}">${label}</button>`).join('')}</div><div id="adminTabBody"></div>`;
    body.querySelectorAll('[data-admin-tab]').forEach(button=>button.onclick=()=>showAdminDashboard(button.dataset.adminTab,target));
    const panel=body.querySelector('#adminTabBody');

    if(tab==='overview')panel.innerHTML=`<div class="stat-grid"><article class="stat-card"><span class="stat-icon">●</span><div><b>${profiles.length}</b><small>Users</small></div></article><article class="stat-card indigo"><span class="stat-icon">▤</span><div><b>${active.length}</b><small>Synced documents</small></div></article><article class="stat-card gold"><span class="stat-icon">V</span><div><b>${active.reduce((n,d)=>n+Number(d.current_version||0),0)}</b><small>Version numbers</small></div></article><article class="stat-card wine"><span class="stat-icon">♲</span><div><b>${deleted.length}</b><small>Deleted documents</small></div></article></div><section class="page-form-card"><h3>Admin scope</h3><p>Only synchronized cloud data appears here. Local-only edits remain private on each user's device until a successful sync.</p><p class="hint">Analytics, conflicts, approvals, restoration, permanent deletion, and the audit log are cloud administration tools. They do not change local autosave or offline editing.</p></section>`;
    if(tab==='analytics')await renderAdminAnalytics(panel,30);
    ...
    if(tab==='versions')await renderAdminVersionIndex(panel);
    if(tab==='conflicts')await renderAdminConflicts(panel,pmap);
    if(tab==='requests')await renderAdminAccountRequests(panel,target);
    if(tab==='audit')await renderAdminAudit(panel,pmap,30);
    if(tab==='trash')await renderAdminTrash(panel,deleted,pmap,target);

    panel.querySelectorAll('.admin-versions').forEach(button=>button.onclick=()=>showAdminVersions(button.dataset.id));
    panel.querySelectorAll('.admin-download').forEach(button=>button.onclick=()=>{const d=(docs||[]).find(x=>x.id===button.dataset.id);if(!d?.document_json)return;downloadJson({format:'TeryaqPackage',packageVersion:'1.0.0',exportedAt:now(),appVersion:APP_VERSION,document:d.document_json},`${safe(d.title||d.id)}.teryaq`);recordAdminEvent('document.admin_download','document',d.id,d.owner_id,{version:d.current_version})});
...
async function renderAdminTrash(panel,deleted,pmap,target){
  const config=await apiFetch('/rest/v1/system_config?key=eq.trash_retention_days&select=value'),retentionDays=Math.max(1,Number(config?.[0]?.value||30)),cutoff=Date.now()-retentionDays*86400000;
  panel.innerHTML=`<section class="page-form-card"><h3>All users' trash</h3><p>Administrators can restore a synchronized document at any time. Permanent deletion is available only after the ${retentionDays}-day retention period and cannot be undone.</p></section><section class="portal-panel">${deleted.length?deleted.map(d=>{const p=pmap.get(d.owner_id)||{},expired=Date.parse(d.deleted_at)<=cutoff,remaining=Math.max(0,Math.ceil((Date.parse(d.deleted_at)+retentionDays*86400000-Date.now())/86400000));return `<div class="trash-row" data-admin-trash="${esc(d.id)}"><div><b>${esc(d.title||d.id)}</b><small>${esc(p.display_name||p.email||d.owner_id)}</small></div><small>Deleted ${esc(new Date(d.deleted_at).toLocaleString())}<br>${expired?'Retention complete':`${remaining} day(s) until purge is allowed`}</small><div class="row-actions"><button class="btn primary small admin-restore" data-id="${esc(d.id)}">Restore</button><button class="btn small admin-download" data-id="${esc(d.id)}" ${d.document_json?'':'disabled'}>Download</button><button class="btn small admin-versions" data-id="${esc(d.id)}">Versions</button><button class="btn danger small admin-purge" data-id="${esc(d.id)}" ${expired?'':'disabled'}>Permanently delete</button></div></div>`}).join(''):'<div class="empty-state">No synchronized deleted documents.</div>'}</section>`;
  ...
  panel.querySelectorAll('.admin-purge').forEach(button=>button.onclick=async()=>{if(prompt('Permanent deletion cannot be undone. Type PURGE to continue:','')!=='PURGE')return;button.disabled=true;try{await apiFetch('/functions/v1/admin-governance',{method:'POST',body:{action:'purge_document',document_id:button.dataset.id}});await showAdminDashboard('trash',target)}catch(error){alert(error.message);button.disabled=false}});
}
...
function renderAuthGate(){
  ...
  byId('accountRequestForm').onsubmit=async event=>{event.preventDefault();const button=byId('submitAccountRequest'),status=byId('accountRequestStatus'),data=Object.fromEntries(new FormData(event.currentTarget).entries());button.disabled=true;button.textContent='Submitting…';status.textContent='';try{await submitAccountRequest(data);status.textContent='Request received. An administrator will review it and send an invitation if approved.';event.currentTarget.reset()}catch(error){status.textContent=error.message}finally{button.disabled=false;button.textContent='Submit request'}};
