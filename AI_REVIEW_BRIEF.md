# TERYAQ Master Tool v2.4.4 — Independent AI Review Brief

## What to upload

Upload the complete `TERYAQ_Master_Tool_AI_Review_v2.4.4.zip`. It contains the application source, bundled JSZip with its license, Supabase migrations, release metadata, test checklists, prior error evidence, and this brief. You may also attach a redacted `.teryaq` export that contains no confidential scientific material.
...
> Act as a senior offline-first PWA and Supabase/PostgreSQL security reviewer. Review the attached TERYAQ Master Tool v2.4.4 source without rewriting the product or weakening its account isolation, RLS, immutable version history, or local-first behavior.
>
