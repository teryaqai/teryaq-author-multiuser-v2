# TERYAQ Master Tool v2.4.2 — Independent AI Review Brief

## What to upload

Upload the complete `TERYAQ_Master_Tool_AI_Review_v2.4.2.zip`. It contains the application source, Supabase migrations, release metadata, test checklists, prior error evidence, and this brief. You may also attach a redacted `.teryaq` export that contains no confidential scientific material.
...
> Act as a senior offline-first PWA and Supabase/PostgreSQL security reviewer. Review the attached TERYAQ Master Tool v2.4.2 source without rewriting the product or weakening its account isolation, RLS, immutable version history, or local-first behavior.
>
...
>
> v2.4.0 retained that sync implementation and changed the document workspace layout. v2.4.1 added document multi-selection, Chapter-number sanitization, password visibility controls, TERYAQ logo rendering, clearer backup descriptions, and conflict-aware Trash behavior. v2.4.2 ensures an editable paragraph follows every new table, adds an editor-only Continue writing control for both new and old tables, and separates Supabase setup from the responsive sign-in card. Verify that these changes do not alter STYLE_CONTRACT, paragraph/inline-mark behavior, table serialization, autosave focus, authentication/account isolation, document identity, queue reconciliation, or backward compatibility.
>
> Trace these exact flows: a delayed push while another autosave commits; repeated edits during Auto Sync; laptop push followed by phone pull; an equal-version pull while the editor is focused; a newer remote pull into a clean open editor; a real two-device version conflict; Keep local after post-conflict editing; delete and restore of a conflicted document both before and after its cloud delete; batch deletion; uninterrupted Arabic/English typing; emergency recovery; ownership repair; offline relaunch; table insertion at the document end; Continue writing on an old final table; linked metadata editing; Print / PDF without editor controls; and service-worker upgrade to v2.4.2.
>
...
>
> End with: (A) whether v2.4.2 preserves v2.3.4 cross-device sync correctness, (B) whether a stale network response can overwrite a newer local save, (C) whether conflict deletion/restoration is logically safe and cannot leave stale active conflicts, (D) whether Continue writing is backward-compatible and excluded from print while v2.3.3 focus behavior and the exact style contract remain preserved, and (E) a prioritized pre-production test list.
