/* TERYAQ Master Tool Platform v2.4.4
...
async function syncNow({silent=false}={}){
  ...
  syncPromise=(async()=>{
    ...
    try{
      ...
      await pullRemote();await settingPut(LAST_SYNC_KEY,now());
      ...
      console.error(e);setSyncLabel('Sync error');const queued=(await all('syncQueue')).filter(x=>x.ownerId===currentUser.id&&x.operation==='upsert');for(const item of queued)await markDocumentSyncError(item.documentId,e.message);if(!silent)alert(`Sync failed: ${e.message}`);return {ok:false,reason:'error',error:e.message}
...
async function initialize(){
  if(!db)throw new Error('Platform DB not attached');const cached=await restoreCachedSession();renderAuthGate();if(cached){await claimAndMigrateLegacyData();await reconcileTrashedConflicts();hideAuthGate();return {authenticated:true,user:cached}}return {authenticated:false}
}
