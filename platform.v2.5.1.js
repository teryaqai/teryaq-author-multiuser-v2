/* TERYAQ Master Tool Platform v2.5.1
...
async function showAccountPage(target='accountPageBody'){
  const body=byId(target);if(!body)return;const name=currentUser?.displayName||'',avatar=await profileAvatarSource();body.innerHTML=`<div class="profile-grid"><section class="page-form-card"><h3>Profile</h3><p>This name and photo are shown in greetings and the account menu.</p><div class="profile-photo-editor"><img id="profileAvatarPreview" src="${esc(avatar)}" alt="Profile"><div><b>${esc(name||'No username yet')}</b><small>${esc(currentUser?.email||'')}</small><div class="row-actions"><button class="btn small" id="chooseProfileAvatar" type="button">Choose photo</button><button class="btn small" id="removeProfileAvatar" type="button">Use default</button><input id="profileAvatarFile" type="file" accept="image/*" hidden></div><small>JPG, PNG, or WebP · maximum 5 MB. The image is center-cropped to a square, saved locally first, then uploaded when online.</small></div></div><div class="field" style="margin-top:16px"><label>Username / Display name</label><input id="profileDisplayName" maxlength="80" value="${esc(name)}" placeholder="Your name"></div><div class="field"><label>Email</label><input value="${esc(currentUser?.email||'')}" readonly></div><div class="modal-actions"><button class="btn primary" id="saveDisplayName">Save profile</button></div><p class="hint" id="displayNameStatus"></p></section><section class="page-form-card"><h3>Change password</h3><p>The new password applies to this Supabase account on every device.</p><div class="field"><label>New password</label><div class="password-field"><input id="newAccountPassword" type="password" autocomplete="new-password" minlength="8">${passwordToggleMarkup('newAccountPasswordToggle')}</div></div><div class="field"><label>Confirm password</label><div class="password-field"><input id="confirmAccountPassword" type="password" autocomplete="new-password" minlength="8">${passwordToggleMarkup('confirmAccountPasswordToggle')}</div></div><button class="btn" id="changePasswordBtn">Change password</button><p class="hint" id="changePasswordStatus">Use at least 8 characters. Internet is required.</p></section></div>`;
  bindPasswordToggle('newAccountPasswordToggle','newAccountPassword');bindPasswordToggle('confirmAccountPasswordToggle','confirmAccountPassword');byId('chooseProfileAvatar').onclick=()=>byId('profileAvatarFile').click();byId('profileAvatarFile').onchange=async event=>{const file=event.target.files[0],status=byId('displayNameStatus');if(!file)return;status.textContent='Saving photo locally…';try{const source=await updateProfileAvatar(file);byId('profileAvatarPreview').src=source;status.textContent=isOnline()?'Photo uploaded and synced.':'Photo saved locally and queued for upload.'}catch(error){status.textContent=error.message}};byId('removeProfileAvatar').onclick=async()=>{await removeProfileAvatar();byId('profileAvatarPreview').src='icons/default-avatar.svg';byId('displayNameStatus').textContent='Default avatar restored.'};byId('saveDisplayName').onclick=async()=>{const button=byId('saveDisplayName'),status=byId('displayNameStatus');button.disabled=true;status.textContent='Saving…';try{const saved=await updateDisplayName(byId('profileDisplayName').value);status.textContent=`Saved as ${saved}.`}catch(e){status.textContent=e.message}finally{button.disabled=false}};byId('changePasswordBtn').onclick=async()=>{const status=byId('changePasswordStatus'),password=byId('newAccountPassword').value,confirmation=byId('confirmAccountPassword').value;if(password!==confirmation){status.textContent='Passwords do not match.';return}const button=byId('changePasswordBtn');button.disabled=true;try{await changePassword(password);byId('newAccountPassword').value='';byId('confirmAccountPassword').value='';status.textContent='Password changed successfully.'}catch(e){status.textContent=e.message}finally{button.disabled=false}}
}
...
async function showAdminDashboard(requested='overview',target='adminPageBody'){
  ...
  const legacy={documents:['operations','documents'],versions:['operations','versions'],conflicts:['operations','conflicts'],requests:['users','requests']},mapped=legacy[requested]||[requested,''],section=mapped[0],sub=mapped[1],sections=[['overview','home','Overview'],['users','users','Users & Access'],['content','templates','Content Setup'],['operations','cloud','Cloud Operations'],['analytics','chart','Analytics'],['audit','activity','Audit Log'],['trash','trash','Trash Management'],['updates','megaphone','Updates']];
  const icon=name=>window.TeryaqUIIcon?.(name)||'';body.innerHTML=`<div class="admin-center-shell"><aside class="admin-section-nav"><div class="admin-nav-title"><span>ADMIN CENTER</span><small>v${APP_VERSION}</small></div>${sections.map(([id,iconName,label])=>`<button class="${section===id?'active':''}" data-admin-section="${id}">${icon(iconName)}<span>${label}</span></button>`).join('')}</aside><section class="admin-section-workspace"><div id="adminSectionBody"><div class="admin-loading">Loading ${esc(sections.find(item=>item[0]===section)?.[2]||'section')}…</div></div></section></div>`;
  body.querySelectorAll('[data-admin-section]').forEach(button=>button.onclick=()=>showAdminDashboard(button.dataset.adminSection,target));const panel=body.querySelector('#adminSectionBody');
  try{await loadAdminSection(section,sub,panel,target);enhanceAdminTables(panel)}catch(error){panel.innerHTML=`<div class="issue error"><b>This section could not load.</b><br>${esc(error.message)}</div>`}
}
