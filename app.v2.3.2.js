...
function updateEditorLocks(){
  ...
  if(editor){
    editor.querySelectorAll('button,select').forEach(el=>{if(busy&&el.dataset.busyWasDisabled===undefined){el.dataset.busyWasDisabled=el.disabled?'1':'0';el.disabled=true}else if(!busy&&el.dataset.busyWasDisabled!==undefined){el.disabled=el.dataset.busyWasDisabled==='1';delete el.dataset.busyWasDisabled}});
    editor.querySelectorAll('input,textarea').forEach(el=>{if(busy&&el.dataset.busyWasReadonly===undefined){el.dataset.busyWasReadonly=el.readOnly?'1':'0';el.readOnly=true}else if(!busy&&el.dataset.busyWasReadonly!==undefined){el.readOnly=el.dataset.busyWasReadonly==='1';delete el.dataset.busyWasReadonly}});
    editor.querySelectorAll('[contenteditable]').forEach(el=>{if(busy&&el.dataset.busyContenteditable===undefined){el.dataset.busyContenteditable=el.getAttribute('contenteditable')||'true';el.setAttribute('contenteditable','false')}else if(!busy&&el.dataset.busyContenteditable!==undefined){el.setAttribute('contenteditable',el.dataset.busyContenteditable);delete el.dataset.busyContenteditable}})
  }
...
async function saveCurrent(manual=false){
  ...
  const task=state.saveChain.then(async()=>{
    try{
      await idbPut('documents',snapshot);await TeryaqPlatform.queueDocument(snapshot);const verified=await idbGet('documents',documentId);if(verified?.localSaveToken!==snapshot.localSaveToken)throw new Error('IndexedDB read-back verification failed');
