...
const BUILTIN_TEMPLATES = [
  ...
  {id:'shooting-script',version:'1.0.0',name:'Shooting Script',category:'Production',editor:'rich-document',description:'Spoken text, script cues and an editable teleprompter.',newDocument(){return createScriptDocument();}}
];
...
function createScriptDocument(){const t=BUILTIN_TEMPLATES.find(x=>x.id==='shooting-script');return {id:uid('doc'),format:'TeryaqDocument',schemaVersion:SCHEMA_VERSION,appVersion:APP_VERSION,templateId:t.id,templateVersion:t.version,title:'Untitled Shooting Script',createdAt:nowIso(),updatedAt:nowIso(),metadata:{},content:{blocks:[normalBlock()]},settings:{teleprompter:{}}}}
...
const MARK_CLASS = {SideNote:'mark-SideNote',NotesToDelete:'mark-NotesToDelete',HighYield:'mark-HighYield',ClinicalCorrelation:'mark-ClinicalCorrelation','script-emphasis':'script-emphasis'};
...
function runsToHtml(runs){
  return normalizeRuns(runs).map(r=>{
    ...
    for(const m of ['SideNote','NotesToDelete','HighYield','ClinicalCorrelation','script-emphasis']) if(marks.includes(m)) h=`<span class="${MARK_CLASS[m]}" data-mark="${m}">${h}</span>`;
...
async function boot(){
  ...
  window.addEventListener('teryaq-content-options-updated',event=>{if(event.detail)state.contentOptions=clone(event.detail);if(state.view==='editor'&&state.current){if(state.current.templateId==='scientific-draft-text')renderMeta();else if(state.current.templateId==='scientific-draft-figures')renderFigureEditor()}});
...
async function refreshUpdates(){
  ...
  const markup=items.length?items.map(item=>`<button type="button" class="update-item ${unread.has(item.id)?'unread':''}" data-update-id="${esc(item.id)}" aria-label="${esc(item.title)}${unread.has(item.id)?' — unread':' — read'}"><span class="update-dot"></span><span><b>${esc(item.title)}</b><p>${esc(item.body)}</p><small>${esc(fmtDate(item.created_at||item.createdAt))}</small></span></button>`).join(''):'<div class="empty-state compact">No updates yet.</div>';
  byId('notificationItems').innerHTML=markup;byId('dashboardUpdates').innerHTML=markup;
}
async function readUpdate(id){
  const entries=$$('[data-update-id]'),visible=[...entries].filter(item=>item.dataset.updateId===id);if(!visible.some(item=>item.classList.contains('unread')))return;
  ...
  try{await TeryaqPlatform.markUpdateRead(id);await refreshUpdates()}catch(error){console.error('Could not mark update as read:',error);await refreshUpdates()}
}
...
function showNewDocumentWizard(selectedId=''){
  ...
  const renderChoice=()=>{
    ...
    byId('wizardCancel').onclick=()=>activateView('documents');byId('wizardNext').onclick=()=>{const t=templates.find(t=>t.id===selected);if(t.id==='shooting-script'||t.baseDocument?.templateId==='shooting-script')void createFromTemplate(t,{});else renderInformation(t)};
  };
...
async function createFromTemplate(t,metadata={}){
  ...
  doc.metadata={...(doc.metadata||{}),...metadata};if(doc.templateId!=='shooting-script')doc.title=[doc.metadata.course||doc.metadata.subject,doc.metadata.chapterNumber?`CH ${doc.metadata.chapterNumber}`:'',doc.metadata.chapterTitle,doc.templateId==='scientific-draft-figures'?'Figures':''].filter(Boolean).join(' — ')||(t.name+' - New');
...
function renderEditor(resetSelection=true){
  ...
  if(d.templateId==='scientific-draft-text')renderTextEditor(resetSelection);else if(d.templateId==='shooting-script')renderScriptEditor(resetSelection);else renderFigureEditor();
...
function renderBlock(block,ctx){
  ...
  if(block.type==='paragraph'){
    ...
    el.classList.add('script-cue',`script-cue-${block.kind}`);const label=document.createElement('span');label.className='script-cue-label';label.textContent=({instruction:'تعليمات لا تُقرأ',silence:'سكوت',repeat:'×2'})[block.kind]||'';el.appendChild(label);if(block.kind==='instruction'){const editor=editableForBlock(block);editor.dataset.placeholder='اكتب تعليمات غير منطوقة…';el.appendChild(editor)}const add=document.createElement('button');add.className='script-cue-add';add.type='button';add.setAttribute('aria-label','Add spoken line after cue');add.textContent='+';add.onclick=()=>{setActiveBlock(block.id);insertSpokenLine()};el.appendChild(add);const remove=document.createElement('button');remove.className='script-cue-remove';remove.type='button';remove.setAttribute('aria-label','Remove cue');remove.textContent='×';remove.onclick=()=>removeBlock(block.id);el.appendChild(remove);
...
function splitCurrentBlock(block,ed){
  ...
  if(block.direction)next.direction=block.direction;const arr=state.current.content.blocks,idx=arr.findIndex(b=>b.id===block.id);arr.splice(idx+1,0,next);markDirty(false);pushHistory('New paragraph');renderBlocks();renderOutline();renderRegisters();setTimeout(()=>focusBlock(next.id,0),0);
}
...
function setRibbonTab(name){
  const previous=state.ribbonTab,tableButton=document.querySelector('[data-ribbon-tab="table"]');if(name==='table'&&tableButton?.disabled)name='styles';if(isScript()&&!['file','formatting','cues','teleprompter'].includes(name))name='formatting';if(state.viewOnly&&name!=='view')name='view';state.ribbonTab=name;
...
function applyMark(mark){
  const b=getBlock();if(!b||!['paragraph','bullet','number'].includes(b.type)||mark==='script-emphasis'&&!isScript())return toast('Select text first');if(!state.selectionBookmark||state.selectionBookmark.blockId!==b.id||state.selectionBookmark.start===state.selectionBookmark.end)return toast('Select text first');
...
function runsPrintHtml(runs){return normalizeRuns(runs).map(r=>{let h=esc(r.text).replace(/\n/g,'<br>');const m=r.marks||[];if(m.includes('bold'))h=`<strong>${h}</strong>`;if(m.includes('italic'))h=`<em>${h}</em>`;if(m.includes('underline'))h=`<u>${h}</u>`;if(m.includes('script-emphasis'))h=`<span class="script-emphasis">${h}</span>`;for(const x of ['SideNote','NotesToDelete','HighYield','ClinicalCorrelation'])if(m.includes(x))h=`<span class="${x}">${h}</span>`;const fs=m.find(x=>x.startsWith('font:'));if(fs)h=`<span style="font-size:${esc(fs.split(':')[1])}pt">${h}</span>`;return h}).join('')}
...
const promptPlayback={playing:false,frame:0,last:0,passedSilences:new Set()};
...
function renderScriptEditor(resetSelection=true){
  const blocks=state.current.content.blocks||[];state.current.content.blocks=blocks;renderBlocks();observeWritingPage();
  if(resetSelection){state.activeBlockId=blocks[0]?.id||null;state.selectionBookmark=null}
  byId('scriptTitleInput').value=state.current.title||'';
  byId('scriptTitleInput').oninput=e=>{state.current.title=e.target.value.trim()||'Untitled Shooting Script';byId('docTitle').textContent=state.current.title;markDirty()};
  bindTextControls();setRibbonTab(state.ribbonTab||'formatting');requestAnimationFrame(()=>{syncEditorStickyOffsets();updatePageScale()});
}
function setScriptDirection(direction){const block=getBlock();if(!block||!['paragraph','bullet','number'].includes(block.type))return toast('Select a line of text first');block.direction=direction;markDirty(false);pushHistory('Text direction');renderBlocks();setTimeout(()=>focusBlock(block.id,state.selectionBookmark?.blockId===block.id?state.selectionBookmark.start:plainText(block.runs).length),0)}
function insertScriptCue(kind){
  const cue={id:uid('b'),type:'cue',kind,runs:kind==='instruction'?[]:undefined};insertBlockAfterActive(cue);
  if(kind==='repeat'){const blocks=state.current.content.blocks,index=blocks.findIndex(block=>block.id===cue.id);if(!blocks.slice(index+1).some(block=>['paragraph','bullet','number'].includes(block.type))){const next=normalBlock();blocks.splice(index+1,0,next);markDirty(false);pushHistory('Add repeated line');renderBlocks();setTimeout(()=>focusBlock(next.id,0),0)}}
  if(kind==='instruction')setTimeout(()=>focusBlock(cue.id,0),0);
}
...
function bindScriptControls(){
  bindToolbarSafe(byId('scriptRtlBtn'),()=>setScriptDirection('rtl'));
  bindToolbarSafe(byId('scriptLtrBtn'),()=>setScriptDirection('ltr'));
  bindToolbarSafe(byId('scriptInstructionBtn'),()=>insertScriptCue('instruction'));
  bindToolbarSafe(byId('scriptSilenceBtn'),()=>insertScriptCue('silence'));
  bindToolbarSafe(byId('scriptRepeatBtn'),()=>insertScriptCue('repeat'));
  bindToolbarSafe(byId('scriptEmphasisBtn'),()=>applyMark('script-emphasis'));
  bindToolbarSafe(byId('scriptLineBtn'),insertSpokenLine);
  ...
  document.onfullscreenchange=()=>byId('scriptTeleprompter').classList.toggle('script-is-fullscreen',document.fullscreenElement===byId('scriptTeleprompter'));
}
...
async function togglePromptFullscreen(){const host=byId('scriptTeleprompter');if(host.classList.contains('script-fallback-fullscreen')){host.classList.remove('script-fallback-fullscreen','script-is-fullscreen');applyPromptSettings();return}try{if(document.fullscreenElement===host)await document.exitFullscreen();else if(host.requestFullscreen)await host.requestFullscreen();else throw new Error('Unsupported');requestAnimationFrame(applyPromptSettings)}catch(error){host.classList.add('script-fallback-fullscreen','script-is-fullscreen');requestAnimationFrame(applyPromptSettings)}}
function renderTeleprompter(){
  if(!isScript())return;
  stopPromptPlayback();const track=byId('scriptPromptTrack'),stage=byId('scriptPromptStage');track.replaceChildren();let repeatNext=0,num=0;
  for(const block of state.current.content.blocks||[]){
    if(block.type==='cue'){
      ...
      if(block.kind==='repeat')repeatNext++;
      continue;
    }
    if(!['paragraph','bullet','number'].includes(block.type))continue;
    const line=document.createElement('div');line.className=`script-prompt-line script-prompt-${block.type}`;line.dir=block.direction||textDirection(plainText(block.runs||[]));line.dataset.sourceId=block.id;
    ...
    const editor=document.createElement('div');editor.className='script-prompt-editable';editor.contentEditable='true';editor.spellcheck=true;editor.dataset.placeholder='Type your spoken line…';editor.innerHTML=runsToHtml(block.runs||[]);
    editor.oninput=()=>{block.runs=elementToRuns(editor);markDirty();for(const duplicate of track.querySelectorAll(`.script-prompt-repeat[data-repeat-of="${CSS.escape(block.id)}"]`)){const target=duplicate.querySelector('.script-prompt-editable');if(target)target.innerHTML=runsToHtml(block.runs)}};
    ...
    while(repeatNext>0){const duplicate=line.cloneNode(true);duplicate.classList.add('script-prompt-repeat');duplicate.dataset.repeatOf=block.id;duplicate.removeAttribute('data-source-id');duplicate.querySelectorAll('[contenteditable]').forEach(node=>node.removeAttribute('contenteditable'));const badge=document.createElement('span');badge.className='script-repeat-badge';badge.textContent='×2';duplicate.prepend(badge);track.appendChild(duplicate);repeatNext--}
  }
  // Repeated lines are derived from the source, so they never create a second editable copy.
...
function togglePromptPlayback(){
  ...
  promptPlayback.playing=true;promptPlayback.last=0;for(const id of ['scriptPlayBtn','scriptPromptFloatPlay'])byId(id).textContent='❚❚ Pause';
  const stage=byId('scriptPromptStage');
  const tick=timestamp=>{
    ...
    stage.scrollTop+=(delta/1000)*34*(Number(scriptSettings().speed)||1);
    ...
    if(stage.scrollTop+stage.clientHeight>=stage.scrollHeight-2){stopPromptPlayback();return}
    promptPlayback.frame=requestAnimationFrame(tick);
  };
  promptPlayback.frame=requestAnimationFrame(tick);
}
function buildScriptPrintHtml(d){
  let body='',number=0;for(const b of d.content.blocks||[]){
    ...
    body+=`<div class="script-line" dir="${direction}">${prefix?`<span class="script-marker">${prefix}</span>`:''}${runsPrintHtml(b.runs||[])}</div>`;
  }
  const css=`${basePrintCss()}body{font-size:16pt;line-height:1.6}.script-line{margin:0 0 12pt;white-space:pre-wrap;overflow-wrap:anywhere}.script-marker{margin-inline-end:10pt;color:#327762}.script-instruction{padding:8pt 12pt;margin:10pt 0;background:#e5f5ed;color:#346c56;border-inline-start:3pt solid #71b596;font-size:11pt}.script-silence{text-align:center;border-block:2pt solid #80bda1;padding:6pt;margin:20pt 0;color:#327762}.script-repeat span{display:inline-block;color:#327762;font-weight:bold;margin-inline-end:8pt}.script-emphasis{color:#147753;font-weight:bold;border-bottom:2pt solid #80bda1}`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(d.title)}</title><style>${css}</style></head><body><main><h1>${esc(d.title)}</h1>${body}</main></body></html>`;
}
