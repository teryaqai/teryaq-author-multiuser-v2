...
const BUILTIN_TEMPLATES = [
  ...
  {id:'shooting-script',version:'1.1.0',name:'Shooting Script',category:'Production',editor:'rich-document',informationTitle:'Shooting Script information',description:'Spoken text, script cues and an editable teleprompter.',newDocument(){return createScriptDocument();}}
];
...
const MARK_CLASS = {SideNote:'mark-SideNote',NotesToDelete:'mark-NotesToDelete',HighYield:'mark-HighYield',ClinicalCorrelation:'mark-ClinicalCorrelation','script-emphasis':'script-emphasis','script-repeat':'script-repeat-inline'};
...
function runsToHtml(runs){
  return normalizeRuns(runs).map(r=>{
    ...
    for(const m of ['SideNote','NotesToDelete','HighYield','ClinicalCorrelation','script-emphasis','script-repeat']) if(marks.includes(m)) h=`<span class="${MARK_CLASS[m]}" data-mark="${m}">${h}</span>`;
...
function templateInformationFields(t){
  const script=t.id==='shooting-script'||t.baseDocument?.templateId==='shooting-script',metadata=script?{...blankMeta(),...(t.baseDocument?.metadata||{})}:t.custom?(t.baseDocument?.metadata||{}):(t.id==='scientific-draft-figures'?{generalNotes:''}:blankMeta()),fields=[{key:'name',label:script?'Script writer':'Author',type:'author',required:true}];
...
async function createFromTemplate(t,metadata={}){
  ...
  doc.metadata={...(doc.metadata||{}),...metadata};doc.title=[doc.metadata.course||doc.metadata.subject,doc.metadata.chapterNumber?`CH ${doc.metadata.chapterNumber}`:'',doc.metadata.chapterTitle,doc.templateId==='scientific-draft-figures'?'Figures':doc.templateId==='shooting-script'?'Shooting Script':''].filter(Boolean).join(' — ')||(t.name+' - New');
...
function updateDocumentTitle(){const m=state.current.metadata;state.current.title=[m.course||m.subject,m.chapterNumber?`CH ${m.chapterNumber}`:'',m.chapterTitle,isScript()?'Shooting Script':''].filter(Boolean).join(' — ')||(isScript()?'Untitled Shooting Script':'Untitled Scientific Draft');byId('docTitle').textContent=state.current.title;if(isScript())byId('scriptTitleInput').value=state.current.title}
...
function applyMark(mark){
  const bookmark=state.selectionBookmark,b=getBlock(bookmark?.blockId);if(!b||!['paragraph','bullet','number'].includes(b.type)||mark==='script-emphasis'&&!isScript()||!bookmark||bookmark.start===bookmark.end)return toast('Select text first');
...
function runsPrintHtml(runs){return normalizeRuns(runs).map(r=>{let h=esc(r.text).replace(/\n/g,'<br>');const m=r.marks||[];if(m.includes('bold'))h=`<strong>${h}</strong>`;if(m.includes('italic'))h=`<em>${h}</em>`;if(m.includes('underline'))h=`<u>${h}</u>`;if(m.includes('script-emphasis'))h=`<span class="script-emphasis">${h}</span>`;if(m.includes('script-repeat'))h=`<span class="script-repeat-inline">${h}</span>`;for(const x of ['SideNote','NotesToDelete','HighYield','ClinicalCorrelation'])if(m.includes(x))h=`<span class="${x}">${h}</span>`;const fs=m.find(x=>x.startsWith('font:'));if(fs)h=`<span style="font-size:${esc(fs.split(':')[1])}pt">${h}</span>`;return h}).join('')}
...
function setScriptAlignment(alignment){const block=getBlock();if(!block||!['paragraph','bullet','number'].includes(block.type))return toast('Select a spoken line first');block.alignment=alignment;markDirty(false);pushHistory(`Align ${alignment}`);renderBlocks();setTimeout(()=>focusBlock(block.id,state.selectionBookmark?.blockId===block.id?state.selectionBookmark.end:plainText(block.runs).length),0)}
...
function insertInlineRepeat(){
  ...
  markDirty(false);pushHistory('Insert ×2');renderBlocks();setTimeout(()=>focusBlock(block.id,offset+spacer.length+2),0);
}
...
function bindScriptControls(){
  ...
  for(const alignment of ['left','center','right'])bindToolbarSafe(byId(`scriptAlign${alignment[0].toUpperCase()+alignment.slice(1)}`),()=>setScriptAlignment(alignment));
  ...
  bindToolbarSafe(byId('scriptRepeatBtn'),insertInlineRepeat);
...
function renderTeleprompter(){
  ...
  for(const block of state.current.content.blocks||[]){
    ...
    const line=document.createElement('div');line.className=`script-prompt-line script-prompt-${block.type}`;line.dir=block.direction||textDirection(plainText(block.runs||[]));line.dataset.sourceId=block.id;if(['left','center','right'].includes(block.alignment))line.style.textAlign=block.alignment;
    ...
    editor.oninput=()=>{block.runs=elementToRuns(editor);markDirty();captureSelection()};
