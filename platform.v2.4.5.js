/* TERYAQ Master Tool Platform v2.4.5
...
async function getContentOptions({force=false}={}){
  ...
  }catch(error){console.warn('Content options refresh:',error.message);return cached}
}
...
async function queueDocument(doc){if(!doc||!currentUser)return;markDocumentPending(doc);if(doc.sync.status==='conflict'){updateSyncUi();return}await put('syncQueue',{id:`${currentUser.id}:${doc.id}`,ownerId:currentUser.id,documentId:doc.id,operation:'upsert',queuedAt:now()});updateSyncUi();scheduleAutoSync()}
...
function recordEditConflict(sent,item,result){
  return new Promise((resolve,reject)=>{
    const transaction=db.transaction(['documents','conflicts','syncQueue'],'readwrite'),documents=transaction.objectStore('documents'),conflicts=transaction.objectStore('conflicts'),queue=transaction.objectStore('syncQueue');let outcome=null,failure=null;
    const request=documents.get(sent.id);
    request.onsuccess=()=>{try{
      ...
      conflicts.put({id:`${item.ownerId}:${sent.id}`,ownerId:item.ownerId,documentId:sent.id,createdAt:now(),kind:'edit',local:clone(latest),remote:result.server_document,serverVersion:result.current_version});
      queue.delete(item.id);outcome=clone(latest)
    }catch(error){failure=error;transaction.abort()}};
    transaction.oncomplete=()=>resolve(outcome);transaction.onerror=()=>reject(failure||transaction.error);transaction.onabort=()=>reject(failure||transaction.error||new Error('Conflict transaction aborted'))
  })
}
...
function reconcileRemoteDocument(remote){
  return new Promise((resolve,reject)=>{
    ...
    request.onsuccess=()=>{try{
      ...
      if(local){
        ...
        if(['pending','local-only'].includes(localStatus)){
          if(remoteVersion>localVersion){local.sync={...(local.sync||{}),status:'conflict',lastError:'A newer cloud version exists; the local draft was preserved.'};documents.put(local);conflicts.put({id:`${currentUser.id}:${remote.id}`,ownerId:currentUser.id,documentId:remote.id,createdAt:now(),kind:'edit',local:clone(local),remote:clone(remote.document_json),serverVersion:remoteVersion});queue.delete(`${currentUser.id}:${remote.id}`);outcome={status:'conflict',document:clone(local)}}
    ...
    transaction.oncomplete=()=>resolve(outcome);transaction.onerror=()=>reject(failure||transaction.error);transaction.onabort=()=>reject(failure||transaction.error||new Error('Remote reconciliation transaction aborted'))
  })
}
...
async function pullRemote(){
  ...
  for(const r of rows||[]){
    ...
    if(r.deleted_at){
      ...
      else{const trashId=`${currentUser.id}:${r.id}`,existingTrash=await get('trash',trashId),deletedDoc=existingTrash?.document||(r.document_json?migrateDocument(r.document_json,currentUser.id):local);if(deletedDoc)await put('trash',{...(existingTrash||{}),id:trashId,ownerId:currentUser.id,documentId:r.id,document:clone(deletedDoc),deletedAt:r.deleted_at,title:existingTrash?.title||deletedDoc.title||r.id,baseServerVersion:remoteVersion,cloudSynced:true});if(local)await del('documents',r.id)}
      continue
    }
...
async function syncNow({silent=false}={}){
  ...
  syncPromise=(async()=>{
    ...
    try{
      await ensureSessionFresh();setSyncLabel('Checking cloud…');await checkServerCompatibility();await registerDevice();
      ...
      const remainingQueue=(await all('syncQueue')).filter(x=>x.ownerId===currentUser.id),remainingConflicts=(await all('conflicts')).filter(x=>x.ownerId===currentUser.id);
...
async function updateSyncUi(){
  ...
  const q=(await all('syncQueue')).filter(x=>x.ownerId===currentUser.id),conflicts=(await all('conflicts')).filter(x=>x.ownerId===currentUser.id),lastError=await settingGet(LAST_SYNC_ERROR_KEY);await getAutoSyncEnabled();
  const text=syncRunning?'Syncing…':conflicts.length?`${conflicts.length} conflict${conflicts.length===1?'':'s'}`:!isOnline()?(q.length?`${q.length} offline change${q.length===1?'':'s'}`:'Offline · saved locally'):lastError?(q.length?`Sync failed · ${q.length} queued`:'Sync check failed · retrying'):q.length?`${q.length} uploading soon`:'Synced ✓';els.forEach(el=>{el.textContent=text;el.title=lastError?.message||''});if(conflictButton){conflictButton.classList.toggle('hidden',!conflicts.length);conflictButton.textContent=`Conflicts (${conflicts.length})`}
}
...
async function resolveConflicts(documentId=null){
  ...
  for(const c of items){const row=document.createElement('div');row.className='sync-conflict';const title=c.local?.title||c.remote?.title||c.documentId;row.innerHTML=`<div><strong>${esc(title)}</strong><br><small>${esc(c.kind)} · server version ${c.serverVersion||'?'}</small></div><div class="conflict-actions"><button class="btn small keep-local">Keep local</button><button class="btn small use-server">Use server</button><button class="btn small save-both">Save both</button></div>`;
    ...
    row.querySelector('.save-both').onclick=async()=>{const latest=await get('documents',c.documentId),source=latest||c.local;if(source){const copy=clone(source);copy.id=uid('doc');copy.title=(copy.title||'Document')+' — Conflict copy';decorateNewDocument(copy);await put('documents',copy);await queueDocument(copy)}if(c.remote){const d=migrateDocument(c.remote,currentUser.id);d.sync={status:'synced',baseServerVersion:Number(c.serverVersion||0),lastSyncedAt:now(),lastError:null};await put('documents',d)}else await del('documents',c.documentId);await del('conflicts',c.id);await del('syncQueue',`${currentUser.id}:${c.documentId}`);resolveConflicts(documentId);emitDataChanged()};
...
async function showAdminDashboard(tab='overview',target='adminPageBody'){
  ...
  try{const [profiles,docs]=await Promise.all([apiFetch('/rest/v1/profiles?select=id,email,display_name,role&order=email.asc'),apiFetch('/rest/v1/documents?select=id,owner_id,title,template_id,template_version,current_version,updated_at,deleted_at,document_json&order=updated_at.desc')]),pmap=new Map((profiles||[]).map(p=>[p.id,p])),active=(docs||[]).filter(d=>!d.deleted_at),deleted=(docs||[]).filter(d=>d.deleted_at);body.innerHTML=`<div class="admin-tabs">${[['overview','Overview'],['content','Content Options'],['users','Users'],['documents','Documents'],['versions','Versions'],['trash','All Trash']].map(x=>`<button class="admin-tab ${tab===x[0]?'active':''}" data-admin-tab="${x[0]}">${x[1]}</button>`).join('')}</div><div id="adminTabBody"></div>`;body.querySelectorAll('[data-admin-tab]').forEach(b=>b.onclick=()=>showAdminDashboard(b.dataset.adminTab,target));const panel=byId('adminTabBody');
    ...
    if(tab==='content')await renderAdminContentOptions(panel);
...
async function writeContentOption(kind,data,id=''){
  if(!isAdmin())throw new Error('Admin access required.');const tables={author:'content_authors',subject:'content_subjects',chapter:'content_chapters'},table=tables[kind];if(!table)throw new Error('Unknown content option type.');const payload={...data,updated_at:now()},path=`/rest/v1/${table}${id?`?id=eq.${encodeURIComponent(id)}`:''}`;await apiFetch(path,{method:id?'PATCH':'POST',body:payload,headers:{'Prefer':'return=minimal'}});return getContentOptions({force:true})
}
async function renderAdminContentOptions(panel){
  const options=await getContentOptions({force:true}),subjects=options.subjects||[],authors=options.authors||[],chapters=options.chapters||[],subjectChoices=(selected='')=>subjects.map(subject=>`<option value="${esc(subject.id)}" ${subject.id===selected?'selected':''}>${esc(subject.name)}${subject.active===false?' (inactive)':''}</option>`).join('');
  panel.innerHTML=`<section class="page-form-card admin-options-intro"><div><h3>Content Options</h3><p>Add or update the choices used for Author, Subject, Chapter number, and Chapter title. Saved changes become available to signed-in devices automatically—no GitHub or Render update is needed.</p></div><button class="btn" id="refreshContentOptions">Refresh</button></section><div class="admin-options-grid">
  <section class="page-form-card"><h3>Authors</h3><form class="option-add-form" data-add-kind="author"><input name="name" required placeholder="Author name"><input name="sort_order" type="number" value="0" title="Display order"><button class="btn primary" type="submit">Add author</button></form><div class="admin-option-list">${authors.map(author=>`<div class="admin-option-row" data-option-kind="author" data-option-id="${esc(author.id)}"><input data-field="name" value="${esc(author.name)}"><input data-field="sort_order" type="number" value="${Number(author.sort_order||0)}"><label><input data-field="active" type="checkbox" ${author.active!==false?'checked':''}> Active</label><button class="btn small save-option">Save</button></div>`).join('')||'<p class="hint">No authors yet.</p>'}</div></section>
  <section class="page-form-card"><h3>Subjects</h3><form class="option-add-form" data-add-kind="subject"><input name="name" required placeholder="Subject name"><input name="sort_order" type="number" value="0" title="Display order"><button class="btn primary" type="submit">Add subject</button></form><div class="admin-option-list">${subjects.map(subject=>`<div class="admin-option-row" data-option-kind="subject" data-option-id="${esc(subject.id)}"><input data-field="name" value="${esc(subject.name)}"><input data-field="sort_order" type="number" value="${Number(subject.sort_order||0)}"><label><input data-field="active" type="checkbox" ${subject.active!==false?'checked':''}> Active</label><button class="btn small save-option">Save</button></div>`).join('')||'<p class="hint">No subjects yet.</p>'}</div></section>
  <section class="page-form-card admin-chapter-options"><h3>Chapters</h3><form class="option-add-form chapter-add-form" data-add-kind="chapter"><select name="subject_id" required><option value="">Subject…</option>${subjectChoices()}</select><input name="chapter_number" required inputmode="decimal" pattern="[0-9.]*" placeholder="No. e.g., 3"><input name="title" required placeholder="Chapter title"><input name="sort_order" type="number" value="0" title="Display order"><button class="btn primary" type="submit">Add chapter</button></form><div class="admin-option-list">${chapters.map(chapter=>`<div class="admin-option-row chapter-option-row" data-option-kind="chapter" data-option-id="${esc(chapter.id)}"><select data-field="subject_id">${subjectChoices(chapter.subject_id)}</select><input data-field="chapter_number" inputmode="decimal" pattern="[0-9.]*" value="${esc(chapter.chapter_number)}"><input data-field="title" value="${esc(chapter.title)}"><input data-field="sort_order" type="number" value="${Number(chapter.sort_order||0)}"><label><input data-field="active" type="checkbox" ${chapter.active!==false?'checked':''}> Active</label><button class="btn small save-option">Save</button></div>`).join('')||'<p class="hint">No chapters yet. Add a subject first.</p>'}</div></section></div><p class="hint" id="contentOptionsStatus"></p>`;
  const status=byId('contentOptionsStatus'),run=async(button,task)=>{button.disabled=true;const old=button.textContent;button.textContent='Saving…';status.textContent='';try{await task();status.textContent='Saved. The updated choices are available to devices now.';await renderAdminContentOptions(panel)}catch(error){status.textContent=error.message;button.disabled=false;button.textContent=old}};
  byId('refreshContentOptions').onclick=()=>renderAdminContentOptions(panel);
  ...
  panel.querySelectorAll('.option-add-form').forEach(form=>form.onsubmit=event=>{event.preventDefault();const button=form.querySelector('button[type="submit"]'),kind=form.dataset.addKind,data=Object.fromEntries(new FormData(form).entries());data.sort_order=Number(data.sort_order||0);data.active=true;if(kind==='chapter'){data.chapter_number=sanitizeCatalogNumber(data.chapter_number);if(!data.chapter_number)return status.textContent='Chapter number must contain numbers and dots only.'}run(button,()=>writeContentOption(kind,data))});
  panel.querySelectorAll('.save-option').forEach(button=>button.onclick=()=>{const row=button.closest('[data-option-id]'),kind=row.dataset.optionKind,data={};row.querySelectorAll('[data-field]').forEach(input=>{data[input.dataset.field]=input.type==='checkbox'?input.checked:input.value.trim()});data.sort_order=Number(data.sort_order||0);if(kind==='chapter'){data.chapter_number=sanitizeCatalogNumber(data.chapter_number);if(!data.chapter_number)return status.textContent='Chapter number must contain numbers and dots only.'}run(button,()=>writeContentOption(kind,data,row.dataset.optionId))})
}
...
async function initialize(){
  if(!db)throw new Error('Platform DB not attached');await settingPut(AUTO_SYNC_KEY,true);const cached=await restoreCachedSession();renderAuthGate();if(cached){await claimAndMigrateLegacyData();await reconcileTrashedConflicts();hideAuthGate();return {authenticated:true,user:cached}}return {authenticated:false}
}

function renderAuthGate(){
  const gate=byId('authGate');if(!gate)return;gate.classList.remove('hidden');bindPasswordToggle('authPasswordToggle','authPassword');const cfgInputs=()=>({projectUrl:byId('authProjectUrl')?.value,anonKey:byId('authAnonKey')?.value});
  ...
  getConfig().then(cfg=>{if(byId('authProjectUrl'))byId('authProjectUrl').value=cfg.projectUrl||'';if(byId('authAnonKey'))byId('authAnonKey').value=cfg.anonKey||'';const ready=configured(cfg);if(cloudState)cloudState.textContent=ready?'Connection saved on this device':'Configure the Supabase connection';setCloudPanel(!ready)});
  byId('saveInitialCloudConfig').onclick=async()=>{const status=byId('initialCloudConfigStatus');status.textContent='';try{const saved=await saveConfig(cfgInputs());if(!configured(saved))throw new Error('Enter both the Supabase project URL and publishable key.');status.textContent='Connection saved on this device. You can sign in now.';if(cloudState)cloudState.textContent='Connection saved on this device'}catch(e){status.textContent=e.message}};
