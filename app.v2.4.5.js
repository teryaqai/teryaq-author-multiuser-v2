...
const state = {
  ...
  ribbonTab:'home', viewOnly:false, viewZoom:1, pageView:'one', documentActionMenu:null,
...
function updateEditorLocks(){
  ...
  for(const id of ['saveBtn','syncBtnEditor','backBtn','settingsBtnEditor','validateBtn','historyBtn','exportMenuButton','backupBtn','saveTemplateBtn','printBtn','htmlExportBtn']){const button=byId(id);if(button)button.disabled=busy}
...
// ---------------- Models ----------------
function blankMeta(){ return {name:'',authorIds:[],startDate:'',endDate:'',subject:'',subjectId:'',chapterNumber:'',chapterTitle:'',chapterId:'',generalNotes:''}; }
...
async function afterAuthentication(){
  await requestPersistentStorage();await TeryaqPlatform.claimAndMigrateLegacyData();const recovered=await recoverEmergencyDrafts();state.contentOptions=await TeryaqPlatform.getContentOptions();await refreshLibrary();renderHome();TeryaqPlatform.updateSyncUi();
...
function bindGlobal(){
  ...
  document.addEventListener('pointerdown',event=>{if(!event.target.closest('#exportMenu'))closeExportMenu();if(!event.target.closest('.document-action-menu-panel')&&!event.target.closest('.row-more'))closeDocumentActionMenu()});
...
function templateInformationFields(t){
  ...
  const definitions={name:['Author','author'],startDate:['Start date','date'],endDate:['End date','date'],subject:['Subject','subject'],chapterNumber:['Chapter number','chapterNumber'],chapterTitle:['Chapter title','chapterTitle'],generalNotes:['General notes','textarea']};
...
function bindMetadataCatalogFields(ids,metadata,onChanged=()=>{}){
  const authorEl=byId(ids.author),subjectEl=byId(ids.subject),numberEl=byId(ids.chapterNumber),titleEl=byId(ids.chapterTitle);if(!authorEl||!subjectEl||!numberEl||!titleEl)return;
  const authors=activeCatalog('authors'),subjects=activeCatalog('subjects'),chapters=activeCatalog('chapters');
  ...
  authorEl.innerHTML=authors.map(author=>`<option value="${esc(author.id)}" ${chosenAuthorIds.has(author.id)?'selected':''}>${esc(author.name)}</option>`).join('');
  const unmatchedAuthors=legacyAuthorNames.filter(name=>!authors.some(author=>author.name===name));for(const name of unmatchedAuthors)authorEl.insertAdjacentHTML('beforeend',`<option value="legacy:${esc(name)}" selected>${esc(name)} (current)</option>`);
  if(!authorEl.options.length)authorEl.innerHTML='<option value="" disabled>Admin has not added authors yet</option>';
  let subjectId=metadata.subjectId||subjects.find(subject=>subject.name===metadata.subject)?.id||'';
  subjectEl.innerHTML=`<option value="">Select subject…</option>${subjects.map(subject=>`<option value="${esc(subject.id)}">${esc(subject.name)}</option>`).join('')}`;
  if(metadata.subject&&!subjectId){subjectId=`legacy:${metadata.subject}`;subjectEl.insertAdjacentHTML('beforeend',`<option value="${esc(subjectId)}">${esc(metadata.subject)} (current)</option>`)}subjectEl.value=subjectId;
  const populateChapters=(reset=false)=>{
    const selectedSubject=subjectEl.value;if(reset){metadata.chapterId='';metadata.chapterNumber='';metadata.chapterTitle=''}
    const available=chapters.filter(chapter=>chapter.subject_id===selectedSubject);let chapterId=metadata.chapterId||available.find(chapter=>chapter.chapter_number===metadata.chapterNumber&&(!metadata.chapterTitle||chapter.title===metadata.chapterTitle))?.id||'';
    ...
    numberEl.innerHTML=`<option value="">Select number…</option>${available.map(chapter=>`<option value="${esc(chapter.id)}">${esc(chapter.chapter_number)}</option>`).join('')}`;
    titleEl.innerHTML=`<option value="">Select title…</option>${available.map(chapter=>`<option value="${esc(chapter.id)}">${esc(chapter.title)}</option>`).join('')}`;
    if(legacyChapter&&!chapterId){chapterId=legacyChapter.id;numberEl.insertAdjacentHTML('beforeend',`<option value="${esc(chapterId)}">${esc(legacyChapter.chapter_number)} (current)</option>`);titleEl.insertAdjacentHTML('beforeend',`<option value="${esc(chapterId)}">${esc(legacyChapter.title)} (current)</option>`)}
    numberEl.value=chapterId;titleEl.value=chapterId;numberEl.disabled=!selectedSubject||(!available.length&&!legacyChapter);titleEl.disabled=numberEl.disabled
  };
  authorEl.onchange=()=>{const selected=[...authorEl.selectedOptions],known=selected.map(option=>authors.find(author=>author.id===option.value)).filter(Boolean);metadata.authorIds=known.map(author=>author.id);metadata.name=selected.map(option=>option.textContent.replace(/ \(current\)$/,'')).join('، ');onChanged('name')};
  subjectEl.onchange=()=>{const subject=subjects.find(item=>item.id===subjectEl.value);metadata.subjectId=subject?.id||'';metadata.subject=subject?.name||(subjectEl.selectedOptions[0]?.textContent||'').replace(/ \(current\)$/,'');populateChapters(true);onChanged('subject')};
  const chapterChanged=event=>{const chapter=chapters.find(item=>item.id===event.target.value);metadata.chapterId=chapter?.id||'';metadata.chapterNumber=chapter?.chapter_number||(event.target===numberEl?event.target.selectedOptions[0]?.textContent.replace(/ \(current\)$/,''):metadata.chapterNumber)||'';metadata.chapterTitle=chapter?.title||(event.target===titleEl?event.target.selectedOptions[0]?.textContent.replace(/ \(current\)$/,''):metadata.chapterTitle)||'';numberEl.value=event.target.value;titleEl.value=event.target.value;onChanged('chapter')};numberEl.onchange=chapterChanged;titleEl.onchange=chapterChanged;populateChapters(false)
}
function wizardFieldMarkup(field){
  ...
  if(field.type==='author')return `<select ${common} multiple size="4" aria-label="Authors"></select><small class="field-hint">Select one or more authors.</small>`;
  if(['subject','chapterNumber','chapterTitle'].includes(field.type))return `<select ${common}></select>`;
...
function showNewDocumentWizard(selectedId=''){
  ...
  const renderInformation=t=>{const fields=templateInformationFields(t),metadata={name:'',authorIds:[],subject:'',subjectId:'',chapterNumber:'',chapterTitle:'',chapterId:''};body.innerHTML=`<section class="wizard-page-card"><div class="wizard-head"><div><span class="eyebrow">New document</span><h3>${esc(t.informationTitle||'Draft information')}</h3><p>Choose the options prepared by the administrator before opening the editor.</p></div><div class="wizard-steps"><span>1 · Template</span><span class="active">2 · Information</span></div></div><div class="selected-template-summary"><b>${esc(t.name)}</b><span>${esc(t.category||'Custom')}</span></div>${metadataCatalogReady()?'':'<div class="issue warn">Some content lists are empty. Ask an administrator to add authors, subjects, and chapters first.</div>'}<form class="wizard-form" id="documentInfoForm">${fields.map(f=>`<div class="field${f.type==='textarea'?' span-2':''}"><label for="wizard-${esc(f.key)}">${esc(f.label)}${f.required?' *':''}</label>${wizardFieldMarkup(f)}</div>`).join('')}<div class="wizard-footer span-2"><button class="btn" type="button" id="wizardBack">← Back</button><button class="btn primary" type="submit">Create & continue</button></div></form></section>`;bindMetadataCatalogFields({author:'wizard-name',subject:'wizard-subject',chapterNumber:'wizard-chapterNumber',chapterTitle:'wizard-chapterTitle'},metadata);byId('wizardBack').onclick=renderChoice;byId('documentInfoForm').onsubmit=async e=>{e.preventDefault();for(const field of fields.filter(f=>!['author','subject','chapterNumber','chapterTitle'].includes(f.type))){const el=byId(`wizard-${field.key}`);metadata[field.key]=el?.value?.trim?.()||''}if(!metadata.name)return toast('Select at least one author');if(!metadata.subjectId)return toast('Select a subject');if(!metadata.chapterId)return toast('Select a chapter');await createFromTemplate(t,metadata)}};
  renderChoice();
}
...
async function openDocument(id){const d=await idbGet('documents',id);if(!d)return;state.current=d;state.dirty=false;state.editRevision=0;state.savedRevision=0;state.ribbonTab='home';state.viewOnly=false;state.viewZoom=1;state.pageView='one';state.activeTableId=null;state.activeCell=null;resetHistory();await activateView('editor');renderEditor();pushHistory('Open');}
...
function bindExportMenu(){
  ...
  byId('backupBtn').onclick=async()=>{closeExportMenu();if(await saveCurrent(true))exportDocumentPackage(state.current)};
  byId('printBtn').onclick=()=>{closeExportMenu();void saveCurrent(false);printCurrent()};
  byId('saveTemplateBtn').onclick=async()=>{closeExportMenu();if(await saveCurrent(true))saveAsTemplate()};
  byId('htmlExportBtn').onclick=async()=>{closeExportMenu();if(await saveCurrent(true))await exportStandaloneHtml()};
}
...
function bindTextControls(){
  ...
  byId('viewOnlyBtn').onclick=()=>{state.viewOnly=!state.viewOnly;if(state.viewOnly){state.ribbonTab='view';state.viewZoom=1;setOutlineDrawerOpen(false)}applyViewOnlyState();setRibbonTab(state.viewOnly?'view':'home')};
...
function setRibbonTab(name){
  const tableButton=document.querySelector('[data-ribbon-tab="table"]');if(name==='table'&&tableButton?.disabled)name='home';if(state.viewOnly&&name!=='view')name='view';state.ribbonTab=name;
  ...
  $$('[data-ribbon-panel]').forEach(panel=>panel.hidden=panel.dataset.ribbonPanel!==name);const toggle=byId('mobileControlsToggle');if(toggle)toggle.querySelector('span').textContent=`${name[0].toUpperCase()+name.slice(1)} tools`;if(innerWidth<=820)byId('leftPanel').classList.add('mobile-open');requestAnimationFrame(syncEditorStickyOffsets)
}
function updateTableTabAvailability(){const button=document.querySelector('[data-ribbon-tab="table"]'),available=!!(state.activeTableId&&state.activeCell);if(!button)return;button.disabled=!available;button.title=available?'Edit the selected table':'Select a table cell to use table tools';if(!available&&state.ribbonTab==='table')setRibbonTab('home')}
...
function applyViewOnlyState(){const area=byId('textEditorArea'),button=byId('viewOnlyBtn'),editor=byId('editorContentColumn'),preview=byId('printPreviewWorkspace');if(!area||!button||!editor||!preview)return;area.classList.toggle('view-only-mode',state.viewOnly);document.body.classList.toggle('document-view-only',state.viewOnly);button.setAttribute('aria-pressed',String(state.viewOnly));button.classList.toggle('primary',state.viewOnly);button.textContent=state.viewOnly?'✓ View only':'◉ View only';area.querySelectorAll('.editable,.table-cell-editor').forEach(el=>el.contentEditable=state.viewOnly?'false':'true');area.querySelectorAll('.draft-information-card input,.draft-information-card textarea,.draft-information-card select,.table-information-card input').forEach(el=>{if(el.matches('select'))el.disabled=state.viewOnly;else el.readOnly=state.viewOnly});editor.classList.toggle('hidden',state.viewOnly);preview.classList.toggle('hidden',!state.viewOnly);$$('[data-ribbon-tab="home"],[data-ribbon-tab="insert"],[data-ribbon-tab="table"]').forEach(tab=>tab.disabled=state.viewOnly||(tab.dataset.ribbonTab==='table'&&!(state.activeTableId&&state.activeCell)));updatePageViewButtons();if(state.viewOnly)renderPrintPreview();else requestAnimationFrame(updatePageScale)}
...
function previewBlockNode(block,ctx){
  ...
  else if(block.type==='bullet'){node.classList.add('bullet-block',`level-${block.level}`);node.innerHTML=`<span class="bullet-marker">${STYLE_CONTRACT.BulletLevels[block.level]?.marker||'●'}</span><div>${runsPrintHtml(block.runs)}</div>`}
...
function renderPrintPreview(){
  ...
  appendPreviewRows([['Name',esc(m.name||'')],['Start date',esc(m.startDate||'')],['End date',esc(m.endDate||'')],['Subject',esc(m.subject||'')],['Chapter number',esc(m.chapterNumber||'')],['Chapter title',esc(m.chapterTitle||'')]],[],'cover','preview-meta-table');
...
function validateCurrent(){const d=state.current,issues=[];const err=m=>issues.push({level:'error',message:m}),warn=m=>issues.push({level:'warn',message:m});if(!d)return issues;
  if(d.templateId==='scientific-draft-text'){
    ...
    const codes=new Map();for(const f of d.content.figures||[]){if(!f.code)warn('A figure has no code.');else if(!/^[0-9.]+$/.test(f.code))err(`Figure code ${f.code} must contain numbers and dots only.`);if(!f.title)warn(`Figure ${f.code||'?'} has no title.`);if(!f.source)warn(`Figure ${f.code||'?'} has no source.`);if(!f.order)warn(`Figure ${f.code||'?'} has no order.`);if(!f.copyright)warn(`Figure ${f.code||'?'} has no copyright status.`);if(f.code)codes.set(f.code,(codes.get(f.code)||0)+1)}for(const [c,n] of codes)if(n>1)err(`Figure code ${c} is duplicated ${n} times.`)
  }
  return issues;
}
...
function renderFigureEditor(){const d=state.current,m=d.metadata;byId('addFigureBtn').onclick=()=>{d.content.figures.push(blankFigure());markDirty(false);pushHistory('Add figure');renderFigureCards();renderFigureNav();setTimeout(()=>byId(`fig-card-${d.content.figures.length}`)?.scrollIntoView({behavior:'smooth',block:'start'}),0)};const updateTitle=()=>{d.title=[m.subject,m.chapterNumber?`CH ${m.chapterNumber}`:'',m.chapterTitle,'Figures'].filter(Boolean).join(' — ')||'Untitled Figures Draft';byId('docTitle').textContent=d.title;markDirty()};bindMetadataCatalogFields({author:'figMetaName',subject:'figMetaSubject',chapterNumber:'figMetaChapter',chapterTitle:'figMetaTitle'},m,updateTitle);const notes=byId('figMetaNotes');notes.value=m.generalNotes||'';notes.oninput=()=>{m.generalNotes=notes.value;markDirty()};renderFigureCards();renderFigureNav();}
