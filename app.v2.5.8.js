...
function renderEditor(resetSelection=true){
  ...
  closeTableOutline();
  byId('appShell').classList.toggle('editor-text-mode',d.templateId==='scientific-draft-text');
...
function setRibbonTab(name){
  ...
  $$('[data-ribbon-panel]').forEach(panel=>panel.hidden=panel.dataset.ribbonPanel!==name);requestAnimationFrame(syncEditorStickyOffsets)
}
...
function applyViewOnlyState(){const area=byId('textEditorArea'),button=byId('viewOnlyBtn'),editor=byId('editorContentColumn'),preview=byId('printPreviewWorkspace');if(!area||!button||!editor||!preview)return;area.classList.toggle('view-only-mode',state.viewOnly);document.body.classList.toggle('document-view-only',state.viewOnly);button.setAttribute('aria-pressed',String(state.viewOnly));button.classList.toggle('primary',state.viewOnly);button.textContent=state.viewOnly?'✓ View only':'◉ View only';area.querySelectorAll('.editable,.table-cell-editor').forEach(el=>el.contentEditable=state.viewOnly?'false':'true');area.querySelectorAll('.draft-information-card input,.draft-information-card textarea,.draft-information-card select').forEach(el=>{if(el.matches('select'))el.disabled=state.viewOnly;else el.readOnly=state.viewOnly});byId('tableOutlinePanel')?.querySelectorAll('input').forEach(el=>el.readOnly=state.viewOnly);editor.classList.toggle('hidden',state.viewOnly);preview.classList.toggle('hidden',!state.viewOnly);$$('[data-ribbon-tab="styles"],[data-ribbon-tab="formatting"],[data-ribbon-tab="insert"],[data-ribbon-tab="table"]').forEach(tab=>tab.disabled=state.viewOnly);updateTableTabAvailability();updatePageViewButtons();if(state.viewOnly){closeTableOutline();renderPrintPreview()}else requestAnimationFrame(updatePageScale)}
