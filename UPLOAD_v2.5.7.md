# TERYAQ Master Tool v2.5.7 — deployment and review

## Deploy

1. Replace the repository files with the contents of this release folder, including `icons/ui/`, all three `*.v2.5.7.*` assets, and `supabase/migrations/012_service_role_profiles_select.sql`.
2. Commit and deploy to Render. The versioned assets and service worker cache are updated to v2.5.7 to avoid stale CSS and icons. Open the app online once on each device and verify the displayed version.
...
- Open a text document: Back shows a left arrow. The title row and tabs have distinct soft colors, and controls sit on a white row. Changing tabs never shows Details or a disclosure arrow.
- Click one unread announcement from Notifications, then inspect Dashboard (and vice versa): only that one item becomes read and the badge drops by one. Offline clicks queue the read state for the next online refresh.
- Check that Sync and Sync Now show the supplied two-arrow icon.
