...
const BUILTIN_TEMPLATES = [
  ...
  {id:'shooting-script',version:'1.2.0',name:'Shooting Script',category:'Production',editor:'rich-document',informationTitle:'Shooting Script information',description:'Spoken text, script cues and an editable teleprompter.',newDocument(){return createScriptDocument();}}
];
...
function updateDocumentTitle(){const m=state.current.metadata;state.current.title=[m.course||m.subject,m.chapterNumber?`CH ${m.chapterNumber}`:'',m.chapterTitle,isScript()?'Shooting Script':''].filter(Boolean).join(' — ')||(isScript()?'Untitled Shooting Script':'Untitled Scientific Draft');byId('docTitle').textContent=state.current.title}
...
function renderBlock(block,ctx){
  ...
  if(block.type==='paragraph'){
    ...
    el.classList.add('script-cue',`script-cue-${block.kind}`);if(block.kind!=='heading'){const label=document.createElement('span');label.className='script-cue-label';label.textContent=({instruction:'تعليمات لا تُقرأ',silence:'سكوت',repeat:'×2'})[block.kind]||'';el.appendChild(label)}if(['instruction','heading'].includes(block.kind)){const editor=editableForBlock(block);editor.dataset.placeholder=block.kind==='heading'?'اكتب عنوان المقطع…':'اكتب تعليمات غير منطوقة…';if(block.kind==='heading')editor.setAttribute('aria-label','Non-spoken script heading');el.appendChild(editor)}const add=document.createElement('button');add.className='script-cue-add';add.type='button';add.setAttribute('aria-label','Add spoken line after cue');add.textContent='+';add.onclick=()=>{setActiveBlock(block.id);insertSpokenLine()};el.appendChild(add);const remove=document.createElement('button');remove.className='script-cue-remove';remove.type='button';remove.setAttribute('aria-label','Remove cue');remove.textContent='×';remove.onclick=()=>removeBlock(block.id);el.appendChild(remove);
...
function renderScriptEditor(resetSelection=true){
  ...
  bindTextControls();updateScriptBlockButtons(getBlock());setRibbonTab(state.ribbonTab||'formatting');requestAnimationFrame(()=>{syncEditorStickyOffsets();updatePageScale()});
}
...
function insertBodySpaceAfterRepeat(event,editable,block){
  ...
  const node=range.startContainer,marker=(node.nodeType===1?node:node.parentElement)?.closest?.('[data-mark="script-repeat"]');
  ...
  event.preventDefault();const space=document.createTextNode(' ');marker.after(space);
  range.setStart(space,1);range.collapse(true);selection.removeAllRanges();selection.addRange(range);
...
function bindScriptControls(){
  ...
  bindToolbarSafe(byId('scriptHeadingBtn'),()=>insertScriptCue('heading'));
