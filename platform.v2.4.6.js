/* TERYAQ Master Tool Platform v2.4.6
...
async function renderAdminContentOptions(panel,requestedCourseId='',notice=null){
  ...
  <section class="page-form-card"><h3>Courses</h3><form class="option-add-form" data-add-kind="course"><input name="name" required placeholder="Course name"><input name="sort_order" type="number" value="0" title="Display order"><button class="btn primary" type="submit">Add course</button></form><div class="admin-option-list">${courses.map(course=>`<div class="admin-option-row" data-option-kind="course" data-option-id="${esc(course.id)}"><input data-field="name" value="${esc(course.name)}"><input data-field="sort_order" type="number" value="${Number(course.sort_order||0)}"><label><input data-field="active" type="checkbox" ${course.active!==false?'checked':''}> Active</label><button class="btn small save-option">Save</button></div>`).join('')||'<p class="hint">No courses yet.</p>'}</div></section>
  <section class="page-form-card admin-chapter-options"><div class="chapter-course-heading"><div><h3>Chapters by Course</h3><p>${selectedCourse?`Showing chapters for ${esc(selectedCourse.name)}.`:'Add a course first, then add its chapters.'}</p></div><label>Course<select id="chapterCourseFilter" ${courses.length?'':'disabled'}><option value="">Select course…</option>${courseChoices}</select></label></div><form class="option-add-form chapter-add-form" data-add-kind="chapter"><input name="chapter_number" required inputmode="decimal" pattern="[0-9.]*" placeholder="No. e.g., 3" ${selectedCourse?'':'disabled'}><input name="title" required placeholder="Chapter title" ${selectedCourse?'':'disabled'}><input name="sort_order" type="number" value="0" title="Display order" ${selectedCourse?'':'disabled'}><button class="btn primary" type="submit" ${selectedCourse?'':'disabled'}>Add chapter</button></form><div class="admin-option-list">${visibleChapters.map(chapter=>`<div class="admin-option-row chapter-option-row" data-option-kind="chapter" data-option-id="${esc(chapter.id)}"><span class="chapter-course-chip">${esc(selectedCourse?.name||'')}</span><input data-field="chapter_number" inputmode="decimal" pattern="[0-9.]*" value="${esc(chapter.chapter_number)}"><input data-field="title" value="${esc(chapter.title)}"><input data-field="sort_order" type="number" value="${Number(chapter.sort_order||0)}"><label><input data-field="active" type="checkbox" ${chapter.active!==false?'checked':''}> Active</label><button class="btn small save-option">Save</button></div>`).join('')||(selectedCourse?'<p class="hint">No chapters in this course yet.</p>':'<p class="hint">Add a course first.</p>')}</div></section></div>`;
