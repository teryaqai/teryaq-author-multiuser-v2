...
async function updateSyncUi(){
  const els=['syncStatus','syncStatusEditor'].map(byId).filter(Boolean);if(!els.length)return;if(!currentUser){els.forEach(el=>el.textContent='');return}
  ...
  const text=conflicts.length?`${conflicts.length} conflict${conflicts.length===1?'':'s'}`:q.length?(isOnline()?(autoSync?`${q.length} syncing soon`:`${q.length} waiting · auto-sync off`):`${q.length} offline change${q.length===1?'':'s'}`):(isOnline()?(autoSync?'Synced ✓':'Online · auto-sync off'):'Offline · saved locally');els.forEach(el=>el.textContent=text);
}

async function conflictCount(){return currentUser?(await all('conflicts')).filter(c=>c.ownerId===currentUser.id).length:0}
async function resolveConflicts(){
  if(!currentUser)return;const items=(await all('conflicts')).filter(c=>c.ownerId===currentUser.id);const body=byId('modalBody');body.innerHTML='<h3>Sync Conflicts</h3><p>Nothing is overwritten automatically. Choose how each conflict should be resolved.</p>';
  if(!items.length)body.innerHTML+='<p>No conflicts.</p>';
